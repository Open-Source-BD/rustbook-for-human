...
1. **`tools/topics.data.js`** — the 74 topics (slug, title, category, level, summary, description, example code, pitfalls, takeaways, prereq/next links, external doc links). This is the authoritative topic list.
...
3. **`src/<category>/<slug>.md`** — the actual lesson prose, hand-edited after the initial stub is generated. `validate.mjs` only requires `## What & why`, `## The idea, slowly`, `## Your turn`, `## Quick check` (contains `<div class="quiz" data-topic="<slug>">`), and `## Remember this`, but every lesson in practice follows the same fixed section order — match it when adding or editing a lesson: `## What & why` → `## The idea, slowly` → `## Common mistakes` → `## More examples` (4 short scenario-driven subsections, each a `### title`, one-sentence real-world hook, then a code fence — no prose after the code) → `## Your turn` → `## Quick check` → `## Remember this` → `## Go deeper`.
